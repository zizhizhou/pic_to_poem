...
def main():
    ...
    def change_poem(poem_list):
        p = list(poem_list[0])
        p.insert(6, '\n')
        p.insert(14, '\n')
        p.insert(22, '\n')
        poems = ''.join(p)
        poem=tkinter.Message(root,bg="#B0E0E6",text=poems, 
                     width=200,font=('华文新魏',17))
        poem.pack()
        poem.place(x=500,y=150)
        tmp=poem_list.pop(0)
        poem_list.append(tmp)
    def print_poem():
        try:
            path=sfname
            label=cfvg.pic_to_label(path)
            poem_list=tr.label_poem(label)
            change_poem(poem_list)
            #poem=tkinter.Message(root,bg="#B0E0E6",text=poem_list[0], 
            #                    width=200,font=('华文新魏',17))
            button4=Button(root,text='下一首',width=10,command=lambda:change_poem(poem_list),font=('华文楷体',11),bg="#A9A9A9")
            button4.place(x=670,y=230)
